...
export default function FlashcardDeck({ listId, listName, items }: FlashcardDeckProps) {
  const storageKey = `flashcards-${listId}`
  ...
  // Build the session queue: due cards first, then new cards (up to MAX_NEW_PER_SESSION)
  ...
  // Init queue on mount
  useEffect(() => {
    setQueue(buildQueue(cardStates))
  ...
  function handleRating(quality: RatingQuality) {
    ...
    setCardStates(prev => ({ ...prev, [currentCard.id]: updated }))
    ...
    if (nextIndex >= queue.length) {
      // Session done — compute next due
      const allStates = { ...cardStates, [currentCard.id]: updated }
      const futureReviews = Object.values(allStates)
        .map(s => s.nextReview)
        .filter(t => t > Date.now())
        .sort((a, b) => a - b)
      ...
      setSummary({
        total: queue.length,
        hard: sessionStats.hard + (label === 'hard' ? 1 : 0),
        good: sessionStats.good + (label === 'good' ? 1 : 0),
        easy: sessionStats.easy + (label === 'easy' ? 1 : 0),
  ...
  function handleResetProgress() {
    setCardStates({})
    const fresh = items.slice(0, MAX_NEW_PER_SESSION)
    setQueue(fresh)
    setCurrentIndex(0)
    setIsFlipped(false)
    setSessionStats({ hard: 0, good: 0, easy: 0 })
  ...
  if (queue.length === 0 && !summary) {
    ...
    const nextReviews = Object.values(allStates)
      .map(s => s.nextReview)
      .filter(t => t > Date.now())
      .sort((a, b) => a - b)
    const next = nextReviews[0]

    return (
      <div className="max-w-md mx-auto px-4 py-16 text-center">
        <div className="text-5xl mb-4">🎉</div>
        <h2 className="text-xl font-bold text-gray-900 mb-2">All caught up!</h2>
        <p className="text-gray-500 mb-2">No cards due right now.</p>
        {next && (
          <p className="text-sm text-gray-400 mb-6">
            Next review: {new Date(next).toLocaleString()}
          </p>
        )}
        <div className="flex flex-col gap-3">
          <Link
            href={`/vocabulary/${listId}`}
            className="inline-flex justify-center items-center bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 py-2.5 rounded-xl font-medium transition-colors"
          >
            ← Back to list
          </Link>
          <button
      ...
      <div className="max-w-md mx-auto px-4 py-16 text-center">
        ...
        <div className="flex justify-center gap-6 mb-8">
          <div className="text-center">
            <p className="text-2xl font-bold text-red-500">{summary.hard}</p>
            <p className="text-xs text-gray-400 mt-1">😓 Hard</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-yellow-500">{summary.good}</p>
            <p className="text-xs text-gray-400 mt-1">🤔 Good</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-green-500">{summary.easy}</p>
            <p className="text-xs text-gray-400 mt-1">😊 Easy</p>
          </div>
        </div>
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <Link
          href={`/vocabulary/${listId}`}
          className="inline-flex items-center text-gray-500 hover:text-gray-800 text-sm"
        >
      ...
      <div className={`mt-8 transition-all duration-300 ${isFlipped ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
        ...
        <div className="flex gap-3 justify-center">
          <button
            ...
            className="flex-1 max-w-[110px] bg-red-50 hover:bg-red-500 text-red-600 hover:text-white border border-red-200 hover:border-red-500 px-3 py-3 rounded-xl text-sm font-medium transition-all"
          >
            😓 Hard
          </button>
          <button
            onClick={() => handleRating(3)}
            className="flex-1 max-w-[110px] bg-yellow-50 hover:bg-yellow-400 text-yellow-600 hover:text-white border border-yellow-200 hover:border-yellow-400 px-3 py-3 rounded-xl text-sm font-medium transition-all"
          >
            🤔 Good
          </button>
          <button
            onClick={() => handleRating(5)}
            className="flex-1 max-w-[110px] bg-green-50 hover:bg-green-500 text-green-600 hover:text-white border border-green-200 hover:border-green-500 px-3 py-3 rounded-xl text-sm font-medium transition-all"
          >
            😊 Easy
          </button>
        </div>
